# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Qatar

## Classification

- Repository: `cloud-itonami-iso3166-qat`
- ISO 3166: `QAT` (Qatar)
...
- Social impact: [:qatari-sme-market-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Qatari
  public contract
...
- registration walkthrough for Monaqasat (monaqasat.mof.gov.qa), Qatar's
  unified e-tendering and procurement platform operated by the Ministry
  of Finance, including notification sign-up and Contractor/Supplier
  classification
- business/tax registration checklist: Commercial Registration (CR) and
  vendor/supplier classification with the Ministry of Commerce and
  Industry (MOCI); individual ministries (e.g. Customs) may run their own
  supplementary supplier-registration process for their own procurement
- local-content / preferential-procurement navigation: Qatarization and
  local-content preference provisions favoring Qatari-owned suppliers and
  Qatari-manufactured goods on qualifying tenders
...
- every requirement cites the official portal or regulation, never
  ...
  Privacy Protection Law, and no invented amendment history beyond the
  specific amending laws confirmed)
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Qatar).

...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (QAT, clean) ==")
    (println (exec-op actor "t1" {:op :engagement/intake :subject "eng-1"
                                  :patch {:id "eng-1" :operator "Kita Systems QA"}} operator))
    ...
    (println "== jurisdiction/assess eng-4 (sets up qa-entity-missing) ==")
    ...
    (println "== filing/submit eng-4 (qa-entity-missing -> HARD hold) ==")

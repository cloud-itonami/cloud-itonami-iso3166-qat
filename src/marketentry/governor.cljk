(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Qatar-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Qatar-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-qa-entity?
                                       true` (near-universal for Qatar
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(CR/統一資格/MoF e-procurement登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- qa-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-qa-entity? true`, INDEPENDENTLY verify
  `:has-qa-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-qa-entity?` ground truth (most Qatar public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-qa-entity? e))
                 (not (true? (:has-qa-entity? e))))
        [{:rule :qa-entity-missing
          :detail (str subject " はカタール居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (qa-entity-missing-violations request st)
